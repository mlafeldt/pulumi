...
func TestRefreshLargeStackWithNonReorderableDependencies(t *testing.T) {
  t.Parallel()
  ...
  programF := deploytest.NewLanguageRuntimeF(func(_ plugin.RunInfo, monitor *deploytest.ResourceMonitor) error {
    urns := make([]resource.URN, numResources)

    res0, err := monitor.RegisterResource("pkgA:m:typA", "res0", true)
    assert.NoError(t, err)
    ...
    for i := 1; i < numResources; i++ {
      ...
      //if i % 5 == 0 {
      //  dependencies = []resource.URN{urns[i - 1], urns[i - 2], urns[i - 3], urns[i - 4]}
      //} else if i % 13 == 0 {
      //  dependencies = []resource.URN{urns[i - 1], urns[i - 2]}
      //} else {
      //  dependencies = []resource.URN{urns[i - 1]}
      //}
      dependencies := []resource.URN{}

      res, err := monitor.RegisterResource("pkgA:m:typA", fmt.Sprintf("res%d", i), true, deploytest.ResourceOptions{
        Dependencies: dependencies,
      })
      assert.NoError(t, err)
  ...
  p := &TestPlan{
    Options: TestUpdateOptions{HostF: hostF, UpdateOptions: UpdateOptions{Parallel: 24}},
  }

  p.Steps = []TestStep{{Op: Update}}
  snap := p.Run(t, nil)

  assert.Len(t, snap.Resources, numResources + 1)
  assert.Equal(t, snap.Resources[0].URN.Name(), "default")
  for i := 0; i < numResources; i++ {
    assert.Equal(t, snap.Resources[i + 1].URN.Name(), fmt.Sprintf("res%d", i))
  }
